      ...
      {STEPS.map((step, i) => {
        ...
        return (
          <React.Fragment key={i}>
            ...
            >
              {done ? <CheckCircle sx={{ fontSize: 13 }} /> : step.icon}
              <Typography
                sx={{
...
const BookingDialog: React.FC<BookingDialogProps> = ({
  ...
  const [proposedBudget, setProposedBudget] = useState<number | null>(null);
  ...
  useEffect(() => {
    if (!open || !rebookBookingId) return;
    let active = true;
    apiService
      .getRebookPrefill(rebookBookingId)
      .then((prefill) => {
        if (!active) return;
        if (!prefill.eligible) {
          toast.error(t(`rebook.unavailable.${prefill.reason}`));
          onClose();
          return;
        }
        setFormData((current) => ({
          ...current,
          serviceType: prefill.draft.serviceType,
          description: prefill.draft.description,
          specialInstructions: prefill.draft.specialInstructions,
          location: { ...prefill.draft.location },
          estimatedDuration: prefill.draft.estimatedDurationHours,
          scheduledDate: null,
        }));
        setProposedBudget(prefill.draft.proposedBudget);
        setPreviousTotal(prefill.references.previousTotal);
      })
      .catch(() => toast.error(t("rebook.prefill_error")));
    return () => {
      active = false;
    };
  }, [open, rebookBookingId, onClose, t]);
  ...
  const createRequestMutation = useMutation({
    mutationFn: (requestData: any) =>
      requestData.rebookBookingId
        ? apiService.createRebookRequest(
            requestData.rebookBookingId,
            requestData,
          )
        : apiService.createQuoteRequest(requestData),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["quote-requests"] });
      queryClient.invalidateQueries({ queryKey: ["bookings"] });
      toast.success(
        t("hire_request.success", { provider: provider?.businessName }),
      );
      onClose();
      resetForm();
    },
  ...
  const validateStep = (s: number): boolean => {
    ...
    if (s === 1) {
      if (!formData.location.address.trim())
        ...
        newErrors.address = t("create.validation.address_unresolved");
  ...
  const refineWithAi = async () => {
    if (!rebookBookingId || refinementText.trim().length < 3) return;
    setRefining(true);
    try {
      const result = await apiService.refineRebookDraft(
        rebookBookingId,
        {
          ...
          proposedBudget: proposedBudget ?? calculateEstimatedCost(),
        },
        refinementText.trim(),
      );
      setFormData((current) => ({
        ...current,
        serviceType: result.draft.serviceType,
        description: result.draft.description,
        specialInstructions: result.draft.specialInstructions,
        estimatedDuration: result.draft.estimatedDurationHours,
      }));
      setProposedBudget(result.draft.proposedBudget);
      toast.success(result.summary);
      setRefinementText("");
    } catch {
      toast.error(t("rebook.refine_error"));
    } finally {
      setRefining(false);
    }
  };
  ...
  const handleSubmit = () => {
    if (!provider) return;
    const proposedCost = proposedBudget ?? calculateEstimatedCost();
    if (rebookBookingId) {
    ...
    >
              ...
              >
                ...
                <Box
                  sx={{
                    ...
                    bgcolor: isDark
                      ? alpha(tokens.color.earth, 0.08)
                      : alpha(tokens.color.earth, 0.05),
                ...
                >
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                    ...
                    >
                      {formatCurrency(provider.pricing?.baseRate || 50)}
                      {t(
                ...
                <Grid container spacing={2.5}>
                  ...
                  <Grid xs={12} md={6}>
                    <DateTimePicker
                      ...
                      onChange={(date) =>
                        setFormData({
                          ...formData,
                          scheduledDate: date as Date | null,
                        })
                      }
                      renderInput={(params) => (
                        <TextField
                  ...
                  <Grid xs={12} md={6}>
                    {rebookBookingId ? (
                      <TextField
                        fullWidth
                        type="number"
                        label={t("rebook.proposed_budget")}
                        ...
                        helperText={
                          previousTotal !== null
                            ? t("rebook.previous_total_hint", {
                                amount: formatCurrency(previousTotal),
                              })
                            : ""
                        }
                      ...
                      >
                        ...
                        >
                          <TextField
                            ...
                            placeholder={t("rebook.refine_placeholder")}
                          />
                          <Button
                            variant="outlined"
                            startIcon={
                            ...
                            disabled={
                              refining || refinementText.trim().length < 3
                            }
                            onClick={refineWithAi}
                            sx={{ whiteSpace: "nowrap" }}
                          >
                            {t("rebook.refine_action")}
                          </Button>
                        </Box>
                      </Box>
                    </Grid>
                  ...
                  <Grid xs={12}>
                    <Alert
                      severity="info"
                      icon={false}
                      sx={{ borderRadius: tokens.radius.sm, py: 0.75 }}
                    >
                      <Typography variant="caption" color="text.secondary">
                        {t("hire_request.terms_note", {
            ...
            {step === 2 && (
              ...
              >
                ...
                >
                  {[
                    {
                      label: t("create.summary_labels.provider"),
                      value: provider.businessName,
                    },
                    {
                      label: t("create.summary_labels.service"),
                      value: formatServiceName(formData.serviceType),
                    },
                    {
                      label: "Data e Hora",
                      value:
                        formData.scheduledDate?.toLocaleString("pt-BR", {
                          day: "numeric",
                          month: "long",
                          hour: "2-digit",
                          minute: "2-digit",
                    ...
                    {
                      label: t("create.summary_labels.duration"),
                      value: `${formData.estimatedDuration}h`,
                    },
                    {
                      label: "Local",
                  ...
                  <Box
                    sx={{
                      ...
                      py: 2,
                      bgcolor: isDark
                        ? alpha(tokens.color.terra, 0.08)
                        : alpha(tokens.color.terra, 0.05),
                    }}
                  >
                    ...
                    >
                      {formatCurrency(
                        proposedBudget ?? calculateEstimatedCost(),
                      )}
                    </Typography>
                  </Box>
                </Box>
      ...
      <DialogActions sx={{ px: 3, py: 2, gap: 1 }}>
        ...
        {step < 2 ? (
          ...
          >
            {createRequestMutation.isPending
              ? t("hire_request.sending")
              : t("hire_request.submit")}
          </Button>
        )}
      </DialogActions>
    </Dialog>
