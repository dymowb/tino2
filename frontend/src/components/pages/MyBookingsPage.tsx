...
import { apiService, Booking, Provider, QuoteRequest, Quote } from '../../services/api';
...
const ACTIVE_BOOKING_STATUSES = ['pending', 'confirmed', 'in_progress', 'pending_completion', 'in_dispute'];
...
const MyBookingsPage: React.FC = () => {
  ...
  const [stage, setStage] = useState<'all' | 'awaiting' | 'active' | 'done'>('all');
  ...
  const updateStatusMutation = useMutation({
    ...
    onError: (err: any) => toast.error(err?.response?.data?.error || t('bookings:messages.status_updated_error')),
  });
  ...
  const cancelBookingMutation = useMutation({
    ...
    onError: (err: any) => toast.error(err?.response?.data?.error || t('bookings:messages.cancelled_error')),
  });

  const quoteStatusMutation = useMutation({
    mutationFn: ({ quoteId, status, reason }: { quoteId: string; status: 'accepted' | 'rejected' | 'withdrawn'; reason?: string }) =>
  ...
  const handleCancelBooking = () => {
    if (selectedBooking) cancelBookingMutation.mutate({ bookingId: selectedBooking.id, reason: cancelReason });
  };
  ...
  const messageProviderFromQuote = async (quote: Quote) => {
    ...
    try {
      const conv = await apiService.createConversation({
        ...
        metadata: { quoteRequestId: quote.requestId, serviceType: quote.serviceType },
      });
  ...
  const jobs: Job[] = useMemo(() => {
    ...
    if (isProvider) {
      ...
      const bookedQuoteIds = new Set(bookings.map(b => b.quoteId).filter(Boolean) as string[]);
      quotes.forEach(q => {
        if (q.status === 'accepted' || bookedQuoteIds.has(q.id)) return;
        out.push({ kind: 'sentquote', quote: q, ts: new Date(q.updatedAt || q.createdAt).getTime() });
      });
    } else {
      ...
      const bookedRequestIds = new Set(bookings.map(b => b.requestId).filter(Boolean) as string[]);
      requests.forEach(r => {
        if (bookedRequestIds.has(r.id)) return;     // becomes a booking card
        if (r.status !== 'open') return;            // cancelled/closed w/o booking → drop
        out.push({
          kind: 'request', request: r, quotes: quotesByRequest.get(r.id) || [],
    ...
    bookings.forEach(b => out.push({
      kind: 'booking', booking: b, ts: new Date(b.updatedAt || b.createdAt).getTime(),
    }));
  ...
  const jobStage = (j: Job): 'awaiting' | 'active' | 'done' | null => {
    ...
    if (j.kind === 'sentquote') return j.quote.status === 'pending' ? 'awaiting' : 'done';
  ...
  const stageCount = useMemo(() => {
    ...
    jobs.forEach(j => { const s = jobStage(j); if (s) c[s]++; });
    return c;
  }, [jobs]);
      ...
      >
        <Box
          ...
          <Box sx={{
            ...
            <Box>
              <Typography sx={{ fontFamily: tokens.font.display, fontSize: '1.125rem', fontWeight: 500, color: 'text.primary', mb: 0.25 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
              <Typography sx={{ fontFamily: tokens.font.mono, fontSize: '1rem', fontWeight: 600, color: booking.status === 'pending' ? tokens.color.terra : 'text.primary' }}>
          ...
          <Box sx={{ px: 3, py: 2, display: 'flex', flexWrap: 'wrap', gap: 3 }}>
            {(isProvider ? booking.customer?.firstName : booking.provider?.businessName) && (
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
              ...
              <Typography variant="body2" color="text.secondary">{formatDate(booking.scheduledDate)}</Typography>
            </Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
              <LocationOn sx={{ fontSize: 15, color: 'text.disabled' }} />
              <Typography variant="body2" color="text.secondary">{booking.location.address}</Typography>
            </Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
              <AccessTime sx={{ fontSize: 15, color: 'text.disabled' }} />
              <Typography variant="body2" color="text.secondary">
                {booking.estimatedDuration % 60 === 0 ? `${booking.estimatedDuration / 60}h` : `${(booking.estimatedDuration / 60).toFixed(1)}h`}
              </Typography>
            </Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
          ...
          {/* Actions */}
          <Box sx={{ px: 3, pb: 2.5, pt: 1, display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center' }}>
            {booking.status === 'pending' && isProvider && (
              <>
                <Button variant="contained" size="small" startIcon={<CheckCircle fontSize="small" />}
                  onClick={() => handleStatusUpdate(booking.id, 'confirmed')} disabled={updateStatusMutation.isPending}
                ...
                <Button variant="outlined" size="small" startIcon={<Cancel fontSize="small" />}
                  onClick={() => { setSelectedBooking(booking); setShowCancelDialog(true); }} disabled={updateStatusMutation.isPending}
                  sx={{ borderColor: tokens.color.terra, color: tokens.color.terra, '&:hover': { bgcolor: `${tokens.color.terra}0D`, borderColor: tokens.color.terraDark }, borderRadius: tokens.radius.full, px: 2.5 }}>
            ...
            {booking.status === 'confirmed' && isProvider && (
              <Button variant="contained" size="small"
                ...
                onClick={() => bookingAction(booking.id, () => apiService.startBooking(booking.id), t('bookings:messages.service_started'))}
                disabled={actionLoading[booking.id]}
                sx={{ bgcolor: tokens.color.terra, '&:hover': { bgcolor: tokens.color.terraDark }, borderRadius: tokens.radius.full, px: 2.5 }}>
            ...
            {booking.status === 'in_progress' && isProvider && (
              <Button variant="contained" size="small"
                ...
                onClick={() => bookingAction(booking.id, () => apiService.markBookingComplete(booking.id), t('bookings:messages.marked_complete'))}
                disabled={actionLoading[booking.id]}
                sx={{ bgcolor: tokens.color.earth, '&:hover': { bgcolor: tokens.color.earthLight }, borderRadius: tokens.radius.full, px: 2.5 }}>
            ...
            {booking.status === 'pending_completion' && !isProvider && (
              <>
                <Button variant="contained" size="small"
                  ...
                  onClick={() => bookingAction(booking.id, () => apiService.confirmBookingCompletion(booking.id), t('bookings:messages.completion_confirmed'))}
                  disabled={actionLoading[booking.id]}
                  sx={{ bgcolor: tokens.color.earth, '&:hover': { bgcolor: tokens.color.earthLight }, borderRadius: tokens.radius.full, px: 2.5 }}>
                  {t('bookings:actions.confirm_completion')}
                </Button>
                <Button variant="outlined" size="small" startIcon={<Cancel fontSize="small" />}
                  onClick={() => { setSelectedBooking(booking); setShowDisputeDialog(true); }} disabled={actionLoading[booking.id]}
            ...
            {booking.status === 'completed' && !isProvider && (
              booking.reviews && booking.reviews.length > 0 ? (
                <Button variant="outlined" size="small" startIcon={<CheckCircle fontSize="small" />} disabled
                  sx={{ borderColor: tokens.color.stone, color: tokens.color.stone, borderRadius: tokens.radius.full, px: 2.5 }}>
                  {t('bookings:actions.review_sent')}
                </Button>
              ) : (
                <Button variant="outlined" size="small" startIcon={<Star fontSize="small" />}
                  onClick={() => toast(t('bookings:messages.review_coming_soon'))}
                  sx={{ borderColor: tokens.color.gold, color: tokens.color.gold, '&:hover': { bgcolor: `${tokens.color.gold}0D`, borderColor: tokens.color.gold }, borderRadius: tokens.radius.full, px: 2.5 }}>
            ...
            {(booking.status === 'pending' || booking.status === 'confirmed') && !isProvider && (
              <Button variant="text" size="small" startIcon={<Cancel fontSize="small" />}
                onClick={() => { setSelectedBooking(booking); setShowCancelDialog(true); }} disabled={updateStatusMutation.isPending}
            ...
            {!['completed', 'cancelled'].includes(booking.status) && (
              <Button variant="text" size="small" startIcon={<Chat fontSize="small" />}
                onClick={async () => {
                  const otherUserId = isProvider ? booking.customer?.id : booking.provider?.userId;
                  if (!otherUserId) { navigate('/messages'); return; }
                  try {
                    const conv = await apiService.createConversation({
  ...
  const QUOTE_CHIP: Record<string, string> = {
    pending: tokens.color.gold, accepted: tokens.color.earth,
    rejected: tokens.color.terra, withdrawn: tokens.color.stone, expired: tokens.color.stone,
  };
  const renderSentQuoteCard = (quote: Quote, index: number) => {
    ...
    const isDirect = Array.isArray(req?.targetProviderIds) && (req!.targetProviderIds as string[]).length > 0;
    const customerName = `${quote.customer?.firstName || ''} ${quote.customer?.lastName || ''}`.trim();
    return (
      <motion.div key={`sq-${quote.id}`}
        ...
        <Box
          ...
          <Box sx={{ px: 3, pt: 2.5, pb: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 1 }}>
            <Box>
              <Typography sx={{ fontFamily: tokens.font.display, fontSize: '1.125rem', fontWeight: 500, color: 'text.primary', mb: 0.25 }}>
                {quote.serviceType.replace(/_/g, ' ').replace(/(^|\s)(\S)/g, (_, s, c) => s + c.toUpperCase())}
              </Typography>
              <Typography sx={{ fontFamily: tokens.font.mono, fontSize: '0.7rem', color: 'text.disabled' }}>
                {isDirect ? t('bookings:hub.direct_badge') : t('bookings:hub.open_badge')} · #{quote.id.substring(0, 8).toUpperCase()}
              </Typography>
            </Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
              <Typography sx={{ fontFamily: tokens.font.mono, fontSize: '1rem', fontWeight: 600, color: tokens.color.terra }}>
                {formatCurrency(quote.estimatedPrice)}
              </Typography>
              <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 0.6, px: 1.5, py: 0.5, borderRadius: tokens.radius.full, bgcolor: `${chip}1F`, border: `1px solid ${chip}44` }}>
                <Box sx={{ width: 6, height: 6, borderRadius: '50%', bgcolor: chip }} />
                <Typography sx={{ fontFamily: tokens.font.body, fontSize: '0.75rem', fontWeight: 600, color: chip, lineHeight: 1 }}>
                  {quote.status === 'pending' ? t('bookings:hub.awaiting_response') : t(`bookings:hub.qstatus.${quote.status}`, quote.status)}
                </Typography>
              </Box>
            </Box>
          </Box>
          ...
          <Box sx={{ px: 3, py: 2, display: 'flex', flexWrap: 'wrap', gap: 3 }}>
            ...
            {req?.location?.city && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
                <LocationOn sx={{ fontSize: 15, color: 'text.disabled' }} />
                <Typography variant="body2" color="text.secondary">{req.location.city}</Typography>
              </Box>
            )}
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
              <Schedule sx={{ fontSize: 15, color: 'text.disabled' }} />
              <Typography variant="body2" color="text.secondary">
                {t('bookings:hub.valid_until', { date: new Date(quote.validUntil).toLocaleDateString('pt-BR') })}
              </Typography>
            </Box>
          </Box>
          ...
          <Box sx={{ px: 3, pb: 2.5, pt: 1, display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {quote.status === 'pending' && (
              <Button variant="text" size="small" startIcon={<Cancel fontSize="small" />}
                onClick={() => quoteStatusMutation.mutate({ quoteId: quote.id, status: 'withdrawn', reason: 'Provider withdrew quote' })}
  ...
  const stageOptions: Array<{ value: typeof stage; label: string; count: number }> = [
    { value: 'all', label: t('bookings:hub.filter.all'), count: stageCount.all },
    { value: 'awaiting', label: isProvider ? t('bookings:hub.filter.awaiting_provider') : t('bookings:hub.filter.awaiting'), count: stageCount.awaiting },
    { value: 'active', label: t('bookings:hub.filter.active'), count: stageCount.active },
    { value: 'done', label: t('bookings:hub.filter.done'), count: stageCount.done },
  ];

  const chipSx = (selected: boolean) => ({
    px: 2, py: 0.75, borderRadius: tokens.radius.full, cursor: 'pointer',
    fontSize: '0.8125rem', fontWeight: 500, fontFamily: tokens.font.body,
    border: '1px solid', transition: 'all 0.15s ease',
    ...(selected ? {
      bgcolor: tokens.color.earth, borderColor: tokens.color.earth, color: '#fff',
    } : {
      bgcolor: 'transparent',
      borderColor: isDark ? tokens.color.nightBorder : tokens.color.paperDark,
      color: 'text.secondary',
      '&:hover': { borderColor: tokens.color.earth, color: tokens.color.earth },
    }),
  });

  const emptyLabel = stage === 'awaiting' ? t('bookings:hub.empty_awaiting')
    : stage === 'active' ? t('bookings:hub.empty_active')
    : stage === 'done' ? t('bookings:hub.empty_done')
    ...
    <Box sx={{ px: { xs: 2, md: 4 }, py: 4, maxWidth: 900, mx: 'auto' }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', mb: 4, flexWrap: 'wrap', gap: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          {!isProvider && (
            <Button startIcon={<RequestQuote fontSize="small" />} onClick={() => setShowNewRequest(true)} variant="contained"
              sx={{ bgcolor: tokens.color.earth, '&:hover': { bgcolor: tokens.color.earthLight }, borderRadius: tokens.radius.full, px: 2.5 }}>
      ...
      <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 4 }}>
        {stageOptions.map(opt => (
          <Box key={opt.value} onClick={() => setStage(opt.value)} sx={chipSx(stage === opt.value)}>
      ...
      {!isLoading && !error && (
        visibleJobs.length === 0 ? (
          <EmptyState isDark={isDark} label={emptyLabel} hint={stage === 'all' && !isProvider ? t('bookings:list.start_booking_prompt') : undefined} />
        ) : (
          <AnimatePresence>
            <Stack spacing={2}>
              {visibleJobs.map((job, i) => {
                if (job.kind === 'booking') return renderBookingCard(job.booking, i);
                if (job.kind === 'sentquote') return renderSentQuoteCard(job.quote, i);
                return (
                  <motion.div key={`req-${job.request.id}`}
                    initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                    ...
                    <RequestCard
                      ...
                      onAcceptQuote={(q) => quoteStatusMutation.mutate({ quoteId: q.id, status: 'accepted' })}
                      onRejectQuote={(q) => quoteStatusMutation.mutate({ quoteId: q.id, status: 'rejected', reason: 'Customer chose another provider' })}
                      onMessageProvider={messageProviderFromQuote}
                      onCloseRequest={(r) => closeRequestMutation.mutate({ requestId: r.id, reason: 'Customer cancelled' })}
      ...
      <Dialog open={showCancelDialog} onClose={() => setShowCancelDialog(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontFamily: tokens.font.display, fontWeight: 500 }}>{t('bookings:cancel.title')}</DialogTitle>
        <DialogContent>
          <Typography variant="body1" sx={{ mb: 2 }}>{t('bookings:cancel.confirm')}</Typography>
          <TextField fullWidth multiline rows={3} label={t('bookings:cancel.reason')} value={cancelReason}
            onChange={(e) => setCancelReason(e.target.value)} variant="outlined" />
        </DialogContent>
        <DialogActions sx={{ p: 2.5 }}>
          <Button onClick={() => setShowCancelDialog(false)} sx={{ borderRadius: tokens.radius.full }}>{t('bookings:actions.keep_booking')}</Button>
          <Button onClick={handleCancelBooking} variant="contained" disabled={cancelBookingMutation.isPending}
            sx={{ bgcolor: tokens.color.terra, '&:hover': { bgcolor: tokens.color.terraDark }, borderRadius: tokens.radius.full }}>
      ...
      <QuoteRequestDialog open={showNewRequest} onClose={() => setShowNewRequest(false)} />

      {/* Dispute Dialog */}
      <Dialog open={showDisputeDialog} onClose={() => setShowDisputeDialog(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontFamily: tokens.font.display, fontWeight: 500 }}>{t('bookings:dispute.title')}</DialogTitle>
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>{t('bookings:dispute.description')}</Typography>
          <TextField fullWidth multiline rows={3} label={t('bookings:dispute.reason_label')} value={disputeReason}
            onChange={(e) => setDisputeReason(e.target.value)} variant="outlined" />
        </DialogContent>
        <DialogActions sx={{ p: 2.5 }}>
          <Button onClick={() => setShowDisputeDialog(false)} sx={{ borderRadius: tokens.radius.full }}>{t('bookings:dispute.cancel')}</Button>
          <Button onClick={() => {
            if (selectedBooking) {
              bookingAction(selectedBooking.id, () => apiService.disputeBooking(selectedBooking.id, disputeReason), t('bookings:dispute.success'));
              setShowDisputeDialog(false);
              setDisputeReason('');
            }
          }} variant="contained" disabled={!disputeReason.trim() || actionLoading[selectedBooking?.id || '']}
            sx={{ bgcolor: tokens.color.terra, '&:hover': { bgcolor: tokens.color.terraDark }, borderRadius: tokens.radius.full }}>
      ...
      {/* Compare quotes */}
      <Dialog open={!!compareQuotes} onClose={() => setCompareQuotes(null)} maxWidth="lg" fullWidth>
        <DialogTitle sx={{ fontFamily: tokens.font.display, fontWeight: 500 }}>{t('bookings:hub.compare')}</DialogTitle>
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            {(compareQuotes || []).map(quote => (
              <Grid item xs={12} md={4} key={quote.id}>
                <Card variant="outlined" sx={{ borderRadius: tokens.radius.md }}>
                  <CardContent>
                    <Typography variant="h6" sx={{ mb: 1 }}>{quote.provider?.businessName || '—'}</Typography>
                    <Typography variant="h4" color="primary" sx={{ mb: 1, fontFamily: tokens.font.mono }}>
                      {formatCurrency(quote.estimatedPrice)}
                    </Typography>
                    ...
                    {quote.provider && (
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
                        ...
                        <Typography variant="caption">({quote.provider.totalReviews})</Typography>
                      </Box>
                    )}
                    <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>{quote.description}</Typography>
                    {quote.status === 'pending' && (
                      <Box sx={{ display: 'flex', gap: 1 }}>
                        <Button size="small" variant="contained" fullWidth disabled={quoteStatusMutation.isPending}
                          onClick={() => quoteStatusMutation.mutate({ quoteId: quote.id, status: 'accepted' })}
                          sx={{ bgcolor: tokens.color.earth, '&:hover': { bgcolor: tokens.color.earthLight }, borderRadius: tokens.radius.full }}>
                          {t('bookings:hub.accept')}
                        </Button>
                        <Button size="small" variant="outlined" color="error" fullWidth disabled={quoteStatusMutation.isPending}
                          onClick={() => quoteStatusMutation.mutate({ quoteId: quote.id, status: 'rejected', reason: 'Customer chose another provider' })}
        ...
        <DialogActions>
          <Button onClick={() => setCompareQuotes(null)}>{t('bookings:dispute.cancel')}</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
...
const EmptyState: React.FC<{ isDark: boolean; label: string; hint?: string }> = ({ isDark, label, hint }) => (
  <Box sx={{
    ...
    {hint && <Typography variant="body2" color="text.disabled" sx={{ mt: 1 }}>{hint}</Typography>}
  </Box>
);
