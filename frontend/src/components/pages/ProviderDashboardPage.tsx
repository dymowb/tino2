  ...
  return (
    <Box
      sx={{
        ...
        bgcolor: isDark
          ? alpha(tokens.color.gold, 0.05)
          : alpha(tokens.color.gold, 0.04),
        ...
        <Box>
          ...
          >
            {booking.serviceType
              ...
              .replace(
                /(^|\s)(\S)/g,
                (_: string, s: string, c: string) => s + c.toUpperCase(),
              )}
          </Typography>
          <Typography variant="caption" color="text.secondary">
            {booking.customer?.firstName || "Cliente"} ·{" "}
...
const ProviderDashboardPage: React.FC = () => {
  ...
  const [availability, setAvailability] =
    useState<AvailabilityForm>(defaultAvailability);
  const [bookingMenuAnchor, setBookingMenuAnchor] =
    ...
    onError: (error: any) => {
      toast.error(
        error?.response?.data?.error ||
          t("dashboard:messages.status_update_error"),
      );
    },
  });
  ...
  const otherBookings = (bookings?.data || []).filter(
    (b: any) => b.status !== "pending",
  );
          ...
          >
            ...
            >
              ...
              <FormControl size="small" sx={{ minWidth: 120 }}>
                ...
                >
                  <MenuItem value="">
                    {t("dashboard:filters.all_status")}
                  </MenuItem>
            ...
            <Box sx={{ p: 2.5 }}>
              ...
              {/* All other bookings */}
              {(statusFilter ? bookings?.data || [] : otherBookings).length ===
                0 &&
              ...
              <Stack spacing={1}>
                {(statusFilter ? bookings?.data || [] : otherBookings).map(
                  (booking: any) => (
                    <Box
                      key={booking.id}
                      sx={{
                    ...
                    >
                      <Box
                        sx={{
                          ...
                          height: 8,
                          borderRadius: "50%",
                          bgcolor: getStatusDot(booking.status),
                          flexShrink: 0,
                        }}
                      />
          ...
          <Stack spacing={3}>
            ...
            >
              ...
              <Box sx={{ display: "flex", gap: 2, mb: 2 }}>
                ...
                <Box>
                  <Typography
                    sx={{ fontWeight: 600, fontFamily: tokens.font.body }}
                  >
                    {user?.firstName} {user?.lastName}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    {providerProfile?.businessName ||
                      t("dashboard:provider.service_provider")}
                  </Typography>
                  <Box
                    sx={{
                      display: "flex",
                  ...
                  >
                    ...
                    >
                      {(() => {
                        const r = parseFloat(
                          String(providerProfile?.rating ?? ""),
                        );
                        return `★ ${isNaN(r) || r === 0 ? "—" : r.toFixed(1)}`;
                      })()}
                    </Typography>
                  </Box>
                </Box>
              ...
              <Stack spacing={1}>
                {[
                    ...
                    text:
                      typeof providerProfile?.location === "string"
                        ...
                        : providerProfile?.location?.address ||
                          t("dashboard:provider.location_not_set"),
                  },
                  {
                    icon: <Phone sx={{ fontSize: 14 }} />,
                    text: user?.phone || t("dashboard:provider.phone_not_set"),
                  },
                  {
                  ...
                  },
                  {
                    ...
                    text:
                      providerProfile?.services?.join(", ") ||
                      t("dashboard:provider.services_not_set"),
                  },
                ].map((item, i) => (
                  <Box
                    key={i}
              ...
              <Box sx={{ mt: 2 }}>
                <ProfileCompletionPrompt
                  completion={providerProfile?.profileCompletion ?? 0}
                  label={t("dashboard:provider.profile_completion")}
                  actionLabel={t("dashboard:provider.complete_profile")}
      ...
      >
        ...
        {selectedBooking?.status === "in_progress" && (
          <MenuItem onClick={() => handleBookingStatusUpdate("completed")}>
            <CheckCircle sx={{ mr: 1 }} />{" "}
            {t("dashboard:actions.mark_complete")}
          </MenuItem>
        )}
        {selectedBooking &&
      ...
      >
        ...
        <DialogContent sx={{ pt: 2 }}>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
            Defina seus horários de trabalho. Clientes só poderão agendar
            durante os horários disponíveis.
          </Typography>
          <Stack spacing={1.5}>
            {DAYS.map((day) => (
              <Box
