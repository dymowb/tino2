...
import AIRequirementsSummary, {
  RequirementsSummary,
} from "./AIRequirementsSummary";
...
const AIAssistantTab: React.FC<AIAssistantTabProps> = ({
  ...
  const { user } = useAuth();
  const favoriteProviders = useFavoriteProviders(user?.userType === "customer");
  const {
    workflow,
    messages,
    isProcessing,
  ...
  const [drawerProvider, setDrawerProvider] = useState<
    import("../../services/api").WorkflowProviderResult | null
  >(null);
  ...
  useEffect(() => {
    ...
    if (summary && !editedRequirements) {
      ...
      setEditedDescription(
        (summary as RequirementsSummary).description ??
          workflow?.context?.userRequest ??
          "",
      );
  ...
  const handleSendQuote = async () => {
    ...
    try {
      ...
      await apiService.createQuoteRequest({
        ...
        preferredDate:
          req.timing.preferredDate &&
          /^\d{4}-\d{2}-\d{2}/.test(req.timing.preferredDate)
            ? req.timing.preferredDate
            : undefined,
        budget: req.budget
          ? {
              min: req.budget.min ?? 0,
              max: req.budget.max ?? 0,
              currency: "BRL",
            }
          : undefined,
        urgency:
          (
            { emergency: "urgent", high: "high", low: "low" } as Record<
              string,
      ...
      toast.success(
        t("results.quoteSent", { count: selectedProviderIds.size }),
      );
      setSelectedProviderIds(new Set());
    } catch {
      toast.error(
    ...
    <Box sx={{ textAlign: "center", py: 4 }}>
      ...
      >
        <TextField
          ...
          placeholder={
            serviceExample
              ? t(`welcome.examples.${serviceExample}`, {
                  defaultValue: t("welcome.placeholder"),
                })
  ...
  const renderProcessing = () => (
    <Box sx={{ textAlign: "center", py: 4 }}>
      ...
      <Typography variant="h6" sx={{ mb: 1 }}>
        {isStreaming
          ? getStatusMessage(progressStage)
  ...
  const renderFollowUp = () => (
    <Box sx={{ maxWidth: 600, mx: "auto", py: 4 }}>
      ...
      {messages
        .filter(
          (msg) =>
            !(msg.role === "assistant" && msg.content === followUpQuestion),
        )
        .map((msg) => (
          <Paper
            key={msg.id}
  ...
  const renderRecommendationCard = (rec: Recommendation) => {
    const colors = rankColors[rec.rank] ?? {
      border: "#E0E0E0",
      badge: "#E0E0E0",
    };
    return (
      <Card
        key={rec.provider.providerId}
        sx={{ mb: 2, border: `2px solid ${colors.border}` }}
      >
        <CardContent>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1 }}>
            ...
            <Rating
              value={Number(rec.provider.rating) || 0}
              readOnly
              size="small"
              precision={0.5}
            />
          ...
          {rec.bestFor && (
            <Chip
              label={`${t("results.bestFor", "Best for")}: ${rec.bestFor}`}
              size="small"
              variant="outlined"
              sx={{ mb: 1.5 }}
            />
        ...
        <Box sx={{ p: 1.5, display: "flex", alignItems: "center", gap: 1 }}>
          <Button
            ...
            onClick={() =>
              setDrawerProvider(
                rec.provider as import("../../services/api").WorkflowProviderResult,
              )
            }
            sx={{ flexShrink: 0 }}
          >
          ...
          <Tooltip
            title={
              selectedProviderIds.size >= MAX_PROVIDERS &&
              !selectedProviderIds.has(rec.provider.providerId)
                ? t("actions.maxSelected")
                : ""
            }
          >
            <span>
              <Chip
                ...
                onClick={
                  ...
                  !selectedProviderIds.has(rec.provider.providerId)
                    ? undefined
                    : () => toggleProvider(rec.provider.providerId)
                }
                color={
                  selectedProviderIds.has(rec.provider.providerId)
                    ? "success"
                ...
                sx={{
                  fontWeight: selectedProviderIds.has(rec.provider.providerId)
                    ? 600
                    : 400,
                }}
              />
        ...
        <AIRequirementsSummary
          ...
          onRerun={() => {
            ...
            if (editedRequirements) {
              startWorkflow(editedDescription, {
                ...editedRequirements,
                description: editedDescription,
              });
            }
      ...
      {recommendations.length > 0 ? (
        <>
          ...
          {verification && !verification.timedOut && (
            ...
            >
              {verification.passed
                ? t("results.qualityVerified", {
                    score: verification.qualityScore,
                  })
                : t("results.qualityConcerns", {
                    score: verification.qualityScore,
                    note:
                      verification.suggestions[0] ?? t("results.someConcerns"),
                  })}
            </Alert>
          )}
          {recommendations
          ...
          {results.length > 0 &&
            (() => {
              const recommendedIds = new Set(
                recommendations.map((r) => r.provider.providerId),
              );
              const remaining = results.filter(
                (p) => !recommendedIds.has(p.providerId),
              );
              if (remaining.length === 0) return null;

              const sorted = [...remaining].sort((a, b) => {
                if (providerSort === "rating") return b.rating - a.rating;
                if (providerSort === "price")
                  return (
                    (a.pricing?.baseRate ?? Infinity) -
                    (b.pricing?.baseRate ?? Infinity)
                  );
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                      ...
                      {(["match", "rating", "price"] as const).map((opt) => (
                        <Button
                          ...
                          size="small"
                          variant={
                            providerSort === opt ? "contained" : "outlined"
                          }
                    ...
                    {sorted.map((provider) => (
                      <Grid item xs={12} md={6} key={provider.providerId}>
                        <AssistantProviderCard
                          provider={provider}
                          analysis={analysisResults.find(
                            (a) => a.providerId === provider.providerId,
                          )}
                          isSelected={selectedProviderIds.has(
                            provider.providerId,
                          )}
                          selectionDisabled={
                            selectedProviderIds.size >= MAX_PROVIDERS
                          ...
                          isFavorite={favoriteProviders.savedIds.has(
                            provider.providerId,
                          )}
                          onToggleFavorite={() =>
                            favoriteProviders.toggle({
                              providerId: provider.providerId,
                              saved: favoriteProviders.savedIds.has(
                                provider.providerId,
                              ),
                            })
                          }
  ...
  return (
    <Box sx={{ pb: selectedProviderIds.size > 0 ? { xs: 20, sm: 10 } : 0 }}>
      ...
      <ProviderDetailDrawer
        ...
        isFavorite={
          !!drawerProvider &&
          favoriteProviders.savedIds.has(drawerProvider.providerId)
        }
        onToggleFavorite={
          drawerProvider
            ? () =>
          ...
          <Box sx={{ flex: 1 }}>
            <Typography variant="body2" fontWeight={600}>
              {t(
                ...
                "{{count}} of {{max}} providers selected",
                {
