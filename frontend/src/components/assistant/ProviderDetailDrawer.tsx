...
import {
  Drawer, Box, Typography, Stack, Chip, Divider, Rating,
  IconButton, Avatar, Button,
} from '@mui/material';
import { Close, Security, Verified, LocationOn, AttachMoney } from '@mui/icons-material';
...
const ProviderDetailDrawer: React.FC<Props> = ({ provider, open, onClose }) => {
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Box sx={{ flex: 1, overflowY: 'auto', p: 3 }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, mb: 3, alignItems: 'flex-start' }}>
            ...
            <Box sx={{ flex: 1 }}>
              <Typography variant="h6" fontWeight={700}>{provider.businessName}</Typography>
              ...
              {Number.isFinite(provider.matchScore) && provider.matchScore > 0 && (
                <Chip
          ...
          {provider.location && (
            <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
              <LocationOn sx={{ color: 'text.secondary', mt: '2px', fontSize: 18 }} />
              <Typography variant="body2" color="text.secondary">
          ...
          {provider.pricing && (
            <Box sx={{ display: 'flex', gap: 1, mb: 3 }}>
              <AttachMoney sx={{ color: 'text.secondary', mt: '2px', fontSize: 18 }} />
              <Typography variant="body2">
                R$ {provider.pricing.baseRate} {t(`providers:card.${provider.pricing.rateType || 'hourly'}`, '/hora')}
              </Typography>
            </Box>
          )}
          ...
          <Stack direction="row" spacing={1} sx={{ mb: 3 }}>
            {provider.isInsured && (
              <Chip icon={<Security fontSize="small" />} label={t('results.insured')} size="small" color="success" variant="outlined" />
            )}
            {provider.isBackgroundChecked && (
              <Chip icon={<Verified fontSize="small" />} label={t('results.backgroundChecked')} size="small" color="info" variant="outlined" />
            )}
          </Stack>
