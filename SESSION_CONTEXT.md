...
### Goal 2 follow-up — minor findings tackled (2026-06-05, user-decided)
After Goal 2, the user asked to address the logged minor findings. Decisions + outcomes (all verified):
- **Suspend → auto-cancel + notify** (decision): `AdminController.updateUserStatus` now, when suspending a PROVIDER, cancels their PENDING/CONFIRMED bookings (reason "Provider suspended by admin") and notifies each customer. Escrow-bearing states (in_progress/pending_completion/in_dispute) are deliberately left for admin/refund handling. Verified: 7 bookings cancelled + PT notifications created. Response includes `cancelledBookings` count.
- **Quote PII → hide until accepted** (decision): `QuoteService.hideCustomerContactForProvider()` strips customer email/phone from provider-facing reads (`searchQuoteRequests` + `getQuoteRequestById`) unless that provider has an ACCEPTED quote on the request. Verified: provider sees contact only on the 3 requests where they have an accepted quote; hidden on pending/withdrawn.
- **No-show → cancellation reason** (decision): `cancelBooking` now persists an optional `reason` (was discarded); route accepts `body('reason')`. Verified: DELETE with `{reason:"no_show"}` → status=cancelled, cancellationReason='no_show'.
- **Backend i18n → full layer** (decision): new `backend/src/i18n/` (`translate`/`t(req,key,params)` + `getLocale` from `X-Locale`/Accept-Language, pt fallback) + `locales/pt.json`+`en.json`. Migrated the audit-scope controllers (AdminController dispute+user-status, BookingController dispute+cancel, AuthController login). Frontend `api.ts` request interceptor sends `X-Locale` from `i18nextLng` (CORS reflects custom headers by default). Verified API PT/EN for dispute/suspend/login AND **UI**: dispute dialog now shows "Nenhum pagamento retido (PaymentIntent) nesta reserva" (was English). **Remaining controllers (payments/messages/providers/users/locations/memory/etc.) still emit English — they adopt the same `t(req, key)` pattern incrementally; catalogs are the single source of truth.**
- **Currency fix**: AdminDisputesPage now formats `R$ 1.234,56` (pt-BR/BRL) in the table + dialog; stripped the duplicate `R$`/`$` from the `hold_amount` i18n templates. Verified UI: "R$ 141,00".
- **Quote-expiry cron**: new `backend/src/jobs/quoteExpiry.job.ts` runs hourly calling `expireOldQuotes`+`expireOldQuoteRequests` (were never scheduled); wired in `server.ts`. Verified: "Quote-expiry cron job scheduled (hourly)" at boot.
- **Middleware reactivation consistency**: `authenticate` lazy-reactivation now also clears suspensionReason/Comment (matches the login path).
- **NOT changed** (logged only): login suspension check still runs before password (account-status enumeration, low sev — left to avoid changing auth UX); notification recipient-locale follows admin's request locale (no per-user locale stored yet).

### GOAL 2 COMPLETE — total defects fixed: Chunk A (6: A1–A6) + Chunk B (2: B1–B2) = **8**. Plus 7 follow-up improvements (suspend-cancel, quote PII, no-show, backend i18n, currency, quote cron, reactivation consistency).
