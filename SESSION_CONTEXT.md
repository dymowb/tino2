...
- **WS2 — Stripe money-movement (✅ verified 2026-06-15):** root cause of the earlier rejection was a stray trailing `n` on the pasted `sk_test_` key. Corrected key in dev `.env`; `verifyStripeFlow` **PASSES** the full escrow lifecycle (HOLD requires_capture → CAPTURE succeeded → REFUND succeeded → CANCEL canceled) against Stripe test mode — same calls PaymentService uses. ⚠️ prod backend still running the old (invalid) value in memory; a `pm2 restart tino-backend` would load the corrected test key (enables TEST-mode Stripe on newtino.com — confirm before doing). Webhook-signature path still needs `STRIPE_WEBHOOK_SECRET`.
