...
router.get(
  ...
  [
    ...
    query('isBackgroundChecked')
      .optional()
      .isBoolean()
      .withMessage('Background check status must be boolean'),
...
router.get('/my', authenticate, requireProviderRole, providerController.getMyProvider);

// Get authenticated provider's profile (alternative path)
router.get('/my/profile', authenticate, requireProviderRole, providerController.getMyProvider);
...
router.post(
  ...
  [
    ...
    body('availableHours.monday.available')
      .isBoolean()
      .withMessage('Monday availability must be boolean'),
    body('availableHours.tuesday.available')
      .isBoolean()
      .withMessage('Tuesday availability must be boolean'),
    body('availableHours.wednesday.available')
      .isBoolean()
      .withMessage('Wednesday availability must be boolean'),
    body('availableHours.thursday.available')
      .isBoolean()
      .withMessage('Thursday availability must be boolean'),
    body('availableHours.friday.available')
      .isBoolean()
      .withMessage('Friday availability must be boolean'),
    body('availableHours.saturday.available')
      .isBoolean()
      .withMessage('Saturday availability must be boolean'),
    body('availableHours.sunday.available')
      .isBoolean()
      .withMessage('Sunday availability must be boolean'),
    body('pricing.baseRate')
      .optional()
      .isFloat({ min: 0 })
      .withMessage('Base rate must be non-negative'),
    body('pricing.currency').optional().isString().withMessage('Currency must be a string'),
    body('pricing.rateType')
      .optional()
