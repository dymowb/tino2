...
router.post(
  ...
  [
    ...
    body('preferredDate')
      .optional()
      .isISO8601()
      .withMessage('Valid preferred date required (ISO 8601 format)'),
    body('budget.min')
      .optional()
      .isFloat({ min: 0 })
    ...
    body('expiresAt')
      .optional()
      .isISO8601()
      .withMessage('Valid expiration date required (ISO 8601 format)'),
...
router.get(
  ...
  [
    ...
    query('sortBy')
      .optional()
      .isIn(['date', 'budget', 'urgency', 'created'])
...
router.get(
  '/requests/:requestId',
  authenticate,
  [param('requestId').isUUID().withMessage('Valid request ID required'), handleValidationErrors],
...
router.post(
  ...
  [
    ...
    body('breakdown.labor')
      .optional()
      .isFloat({ min: 0 })
      .withMessage('Labor cost must be positive'),
    body('breakdown.materials')
      .optional()
      .isFloat({ min: 0 })
      .withMessage('Materials cost must be positive'),
    body('breakdown.equipment')
      .optional()
      .isFloat({ min: 0 })
      .withMessage('Equipment cost must be positive'),
    body('breakdown.other')
      .optional()
      .isFloat({ min: 0 })
      .withMessage('Other cost must be positive'),
...
router.get(
  '/:quoteId',
  authenticate,
  [param('quoteId').isUUID().withMessage('Valid quote ID required'), handleValidationErrors],
...
router.put(
  ...
  [
    ...
    body('validUntil')
      .optional()
      .isISO8601()
      .withMessage('Valid until date required (ISO 8601 format)'),
    body('breakdown.labor')
      .optional()
      .isFloat({ min: 0 })
      .withMessage('Labor cost must be positive'),
