...
async function main() {
  ...
  for (const r of zero) {
    const loc: any = r.location || {};
    const addr = [loc.address, loc.city, loc.state, loc.country || 'Brasil']
      .filter(Boolean)
      .join(', ')
      .trim();
    if (!loc.address) {
      skipped++;
      logger.warn(`Skip ${r.id.slice(0, 8)} — no address`);
      continue;
    }
    try {
      ...
      if (!latitude || !longitude) {
        skipped++;
        logger.warn(`Skip ${r.id.slice(0, 8)} — geocode returned 0`);
        continue;
      }
      ...
      if (geo.partialMatch || geo.locationType === 'APPROXIMATE') {
        skipped++;
        logger.warn(
          `Skip ${r.id.slice(0, 8)} — imprecise (${geo.locationType}, partial=${geo.partialMatch}) for "${addr}"`
        );
        continue;
      }
